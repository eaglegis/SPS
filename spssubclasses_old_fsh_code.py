...
#   arcgispro-py3
#
# FSH - add support for overriding default objects and write out xml as they expect.
...
class inspectionTypeSub(supermod.inspectionType):
    def __init__(self, serialNumber=None, forestManager=None, location=None, stand=None, siteId=None, subSite=None,
                 bioRegion=None, siteType=None, date=None, inspector=None, hostSpec=None, estabYear=None,
                 treatment=None, stemsPerHa=None, diam=None, height=None, inspType=None, sampleType=None, comments=None,
                 points=None, disorders=None, identifications=None):
        super(inspectionTypeSub, self).__init__(serialNumber, forestManager, location, stand, siteId, subSite,
                                                bioRegion, siteType, date, inspector, hostSpec, estabYear, treatment,
                                                stemsPerHa, diam, height, inspType, sampleType, comments, points,
                                                disorders, identifications, )
    ...
    def exportChildren(self, outfile, level, namespace_='', name_='inspectionType', fromsubclass_=False, pretty_print=True):
        if pretty_print:
            ...
            eol_ = ''
        #based on schema we assume that no _ in field names - filters out generateds stuff
        for attr, value in self.__dict__.items():
            print(attr, value)
            if "_" in attr:
                continue
            if attr in ["disorders","identifications"]:
                continue
            else:
                if value is not None:
                    ...
                    outfile.write('<{0}>{1}</{0}>{2}'.format(attr,
                        self.gds_encode(
                            self.gds_format_string(supermod.quote_xml(value), input_name=attr)),
                        eol_))
                else:
                    supermod.showIndent(outfile, level, pretty_print)
...
#FSH added support for empty tags
# same as base method - need to look into generation code
...
class disorderTypeSub(supermod.disorderType):
    def __init__(self, name=None, comments=None, agent=None, aspect=None, terrain=None, position=None, type_=None, severity=None, extent=None, incidencePercent=None, isSampleTaken=None):
        super(disorderTypeSub, self).__init__(name, comments, agent, aspect, terrain, position, type_, severity, extent, incidencePercent, isSampleTaken, )
